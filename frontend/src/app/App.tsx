      ...
      <div className={`relative w-full bg-white text-zinc-900 font-sans z-10 shadow-[0_30px_60px_rgba(0,0,0,0.15)] ${stepIndex >= 7 ? 'min-h-[900px]' : 'h-[900px] overflow-hidden'}`}>
        ...
        {stepIndex >= 7 && (
          <>
            ...
            {/* Global CTA Section placed exactly above footer */}
            <section className="bg-[#111111] w-full py-32 px-8 flex flex-col items-center justify-center text-center relative z-10">
              <h2 className="text-white text-5xl md:text-6xl font-semibold mb-6 tracking-tight">
                Ready to design?
              </h2>
              <p className="text-[#a1a1aa] text-lg md:text-xl mb-12 max-w-2xl mx-auto">
                Join the designers streamlining their packaging workflow.
              </p>
              
              <button className="relative group rounded-full p-[3px] overflow-hidden transition-transform hover:scale-105 active:scale-95">
                {/* Rainbow Border */}
                <div 
                  className="absolute inset-[-100%] animate-spin [animation-duration:3s]" 
                  style={{
                    background: 'conic-gradient(from 0deg, #4285F4, #EA4335, #FBBC05, #34A853, #4285F4)'
                  }}
                />
                {/* Inner Dark Button */}
                <div className="relative bg-[#1a1a1a] group-hover:bg-[#222222] transition-colors rounded-full px-10 py-4 h-full w-full flex items-center justify-center shadow-inner">
                  <span className="text-white font-medium text-lg">Start Designing for Free</span>
                </div>
              </button>
            </section>
          </>
