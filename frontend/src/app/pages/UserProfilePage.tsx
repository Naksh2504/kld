...
function UserProfilePage({ onBack, onNavigate }: { onBack: () => void; onNavigate?: (view: string) => void }) {
  ...
  const fetchSavedProjects = async () => {
    ...
    // Read local workspace items from localforage (fallback / sync)
  ...
  const handleDeleteProject = async (projectId: string) => {
    ...
    // 2. Instantly remove from localforage cache
    try {
      let stored: any = await localforage.getItem('kld_workspace_items');
      if (stored) {
        const parsed = typeof stored === 'string' ? JSON.parse(stored) : stored;
        if (Array.isArray(parsed)) {
          ...
          await localforage.setItem('kld_workspace_items', remaining);
        }
      }
    } catch (e) {
